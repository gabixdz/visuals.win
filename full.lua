...
Visuals["trail"] = {
    name="body trail", desc="glowing trail follows your movement", settings={"color","rainbow","size","opacity"},
    start=function(key)
        ...
        trail.Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,1-getSetting(key,"opacity")),NumberSequenceKeypoint.new(1,1)})
        trail.WidthScale=NumberSequence.new(getSetting(key,"size")*1.5)
        trail.LightEmission=1 trail.FaceCamera=true trail.Parent=hrp
...
Visuals["jumpcircle"] = {
    name="jump circle", desc="ring expands when you jump", settings={"color","rainbow","size"},
    start=function(key)
...
Visuals["shockwave"] = {
    name="landing shockwave", desc="shockwave explodes when you land", settings={"color","rainbow","size","opacity"},
    start=function(key)
...
Visuals["orbit"] = {
    name="orbit particles", desc="glowing orbs orbit around you", settings={"color","rainbow","size","count","speed"},
    start=function(key)
        local hrp=getHRP() if not hrp then return end
...
Visuals["wings"] = {
    name="wings", desc="glowing wings on your back", settings={"color","rainbow","size","opacity"},
    start=function(key)
        local hrp=getHRP() if not hrp then return end
        local wings={}
        for side=-1,1,2 do
            for seg=1,4 do
                ...
                w.Transparency=1-getSetting(key,"opacity") w.Parent=workspace
...
Visuals["groundglow"] = {
    name="ground glow", desc="glowing circle beneath your feet", settings={"color","rainbow","size","opacity"},
    start=function(key)
...
Visuals["speedlines"] = {
    name="speed lines", desc="lines shoot past you when moving fast", settings={"color","rainbow","size","speed"},
    start=function(key)
        local lastPos=nil
        Conns[key]=RunService.Heartbeat:Connect(function()
            local hrp=getHRP() if not hrp then return end
...
Visuals["headaura"] = {
    name="head aura", desc="glowing aura around your head", settings={"color","rainbow","size","opacity"},
    start=function(key)
...
Visuals["footsparks"] = {
    name="footstep sparks", desc="sparks pop under your feet as you walk", settings={"color","rainbow","size"},
    start=function(key)
...
Visuals["nametag"] = {
    name="nametag glow", desc="glowing custom name tag above head", settings={"color","rainbow","size"},
    start=function(key)
        local char=getChar() if not char then return end
        local head=char:FindFirstChild("Head") if not head then return end
        local sg=Instance.new("BillboardGui")
        sg.Size=UDim2.new(0,200*getSetting(key,"size"),0,40)
        sg.StudsOffset=Vector3.new(0,3.5,0) sg.Adornee=head sg.AlwaysOnTop=true sg.Parent=head
        local lbl=Instance.new("TextLabel")
        lbl.Size=UDim2.new(1,0,1,0) lbl.BackgroundTransparency=1
        lbl.Text="✦ "..player.Name.." ✦"
        lbl.Font=Enum.Font.GothamBold lbl.TextSize=16*getSetting(key,"size")
        lbl.TextColor3=getSetting(key,"color") lbl.TextStrokeTransparency=0.3 lbl.Parent=sg
        Objects[key]={sg}
        if getSetting(key,"rainbow") then
            RainbowC[key]=RunService.Heartbeat:Connect(function()
                if not Enabled[key] then stopRainbow(key) return end
...
Visuals["chams"] = {
    name="body outline", desc="neon outline around your entire body", settings={"color","rainbow","size","opacity"},
    start=function(key)
        local char=getChar() if not char then return end
        local parts={}
...
Visuals["runes"] = {
    name="floating runes", desc="mystical rune symbols float around you", settings={"color","rainbow","size","count"},
    start=function(key)
        ...
        for i=1,n do
            ...
            lbl.TextColor3=getSetting(key,"color") lbl.TextStrokeTransparency=0.2 lbl.Parent=bb
            table.insert(runes,{part=part,lbl=lbl})
        end
        local objList={}
        for _,r in ipairs(runes) do table.insert(objList,r.part) end
        Objects[key]=objList
        local t0=tick()
        Conns[key]=RunService.Heartbeat:Connect(function()
            local hrp=getHRP() if not hrp then return end
...
Visuals["lightning"] = {
    name="lightning bolts", desc="lightning crackles around you", settings={"color","rainbow","size","speed"},
    start=function(key)
        local timer=0
        Conns[key]=RunService.Heartbeat:Connect(function(dt)
            local hrp=getHRP() if not hrp then return end
...
Visuals["smoke"] = {
    name="smoke trail", desc="dreamy smoke follows your movement", settings={"color","rainbow","size","opacity","speed"},
    start=function(key)
        local timer=0
        Conns[key]=RunService.Heartbeat:Connect(function(dt)
            local hrp=getHRP() if not hrp then return end
...
Visuals["petals"] = {
    name="petal rain", desc="flower petals drift around you", settings={"color","rainbow","size","count"},
    start=function(key)
...
Visuals["starburst"] = {
    name="star burst", desc="stars explode outward periodically", settings={"color","rainbow","size","speed"},
    start=function(key)
        local timer=0
        Conns[key]=RunService.Heartbeat:Connect(function(dt)
            local hrp=getHRP() if not hrp then return end
...
Visuals["energyrings"] = {
    name="energy rings", desc="spinning energy rings surround you", settings={"color","rainbow","size","count","speed"},
    start=function(key)
        ...
        for i=1,n do
            ...
            local mesh=Instance.new("SpecialMesh") mesh.MeshType=Enum.MeshType.Cylinder mesh.Scale=Vector3.new(0.04,1,1) mesh.Parent=ring
...
Visuals["ghost"] = {
    name="ghost echo", desc="transparent ghost copies trail behind you", settings={"color","rainbow","opacity","speed"},
    start=function(key)
        local timer=0
        Conns[key]=RunService.Heartbeat:Connect(function(dt)
            local char=getChar() if not char then return end
...
Visuals["bubbles"] = {
    name="bubble float", desc="iridescent bubbles float up around you", settings={"color","rainbow","size","count"},
    start=function(key)
        local timer=0
        Conns[key]=RunService.Heartbeat:Connect(function(dt)
            local hrp=getHRP() if not hrp then return end
...
Visuals["firecrown"] = {
    name="fire crown", desc="fiery crown of flames on your head", settings={"color","rainbow","size","count"},
    start=function(key)
        local char=getChar() if not char then return end
        local head=char:FindFirstChild("Head") if not head then return end
...
Visuals["icecrystals"] = {
    name="ice crystals", desc="sharp ice shards orbit around you", settings={"color","rainbow","size","count","speed"},
    start=function(key)
...
Visuals["neongrid"] = {
    name="neon grid", desc="neon grid platform under your feet", settings={"color","rainbow","size","opacity"},
    start=function(key)
        ...
        local objList={}
        for _,l in ipairs(lines) do table.insert(objList,l.part) end
        Objects[key]=objList
        Conns[key]=RunService.Heartbeat:Connect(function()
            local hrp=getHRP() if not hrp then return end
            ...
            for _,l in ipairs(lines) do
                ...
                if l.axis=="x" then l.part.CFrame=CFrame.new(base+Vector3.new(0,0,l.offset))
                else l.part.CFrame=CFrame.new(base+Vector3.new(l.offset,0,0)) end
...
Visuals["comet"] = {
    name="comet tail", desc="bright comet-like streak trails you", settings={"color","rainbow","size","opacity"},
    start=function(key)
        local hrp=getHRP() if not hrp then return end
        ...
        trail.Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,1-getSetting(key,"opacity")),NumberSequenceKeypoint.new(1,1)})
        trail.WidthScale=NumberSequence.new({NumberSequenceKeypoint.new(0,getSetting(key,"size")*3),NumberSequenceKeypoint.new(1,0)})
        trail.Parent=hrp
        ...
        if getSetting(key,"rainbow") then
            RainbowC[key]=RunService.Heartbeat:Connect(function()
                if not Enabled[key] then stopRainbow(key) return end
                local c=rainbowColor()
                trail.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,c),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(255,255,255)),ColorSequenceKeypoint.new(1,c)})
            end)
        end
    end,
    stop=function(key) stopRainbow(key) cleanObjects(key) end,
...
Visuals["voidripple"] = {
    name="void ripple", desc="dark energy ripples expand from you", settings={"color","rainbow","size","speed"},
    start=function(key)
        local timer=0
        Conns[key]=RunService.Heartbeat:Connect(function(dt)
            local hrp=getHRP() if not hrp then return end
...
Visuals["halo"] = {
    name="angel halo", desc="glowing halo floats above your head", settings={"color","rainbow","size","opacity"},
    start=function(key)
...
Visuals["dragon"] = {
    name="dragon aura", desc="fierce dragon-like energy surrounds you", settings={"color","rainbow","size","opacity","speed"},
    start=function(key)
...
Visuals["musicbars"] = {
    name="music bars", desc="equalizer bars bounce around you", settings={"color","rainbow","size","count","speed"},
    start=function(key)
...
Visuals["glitch"] = {
    name="glitch effect", desc="your body glitches and fragments", settings={"color","rainbow","opacity","speed"},
    start=function(key)
        local timer=0
        Conns[key]=RunService.Heartbeat:Connect(function(dt)
            local char=getChar() if not char then return end
...
Visuals["constellation"] = {
    name="constellation", desc="stars connected by lines orbit you", settings={"color","rainbow","size","count"},
    start=function(key)
        ...
        local objList={}
        for _,s in ipairs(stars) do table.insert(objList,s) end
        for _,b in ipairs(beams) do table.insert(objList,b) end
        Objects[key]=objList
        ...
        Conns[key]=RunService.Heartbeat:Connect(function()
            ...
            for i,star in ipairs(stars) do
                ...
                star.CFrame=CFrame.new(pos) positions[i]=pos
...
Visuals["deathexplosion"] = {
    name="death explosion", desc="epic explosion when you die", settings={"color","rainbow","size"},
    start=function(key)
        local char=getChar() if not char then return end
        local hum=getHum() if not hum then return end
...
Visuals["spiralvortex"] = {
    name="spiral vortex", desc="particles spiral up from the ground", settings={"color","rainbow","size","count","speed"},
    start=function(key)
...
Visuals["meteors"] = {
    name="meteor shower", desc="meteors streak down around you", settings={"color","rainbow","size","speed"},
    start=function(key)
        local timer=0
        Conns[key]=RunService.Heartbeat:Connect(function(dt)
            local hrp=getHRP() if not hrp then return end
...
Visuals["rainbowaura"] = {
    name="rainbow aura", desc="full-body rainbow glow aura", settings={"size","opacity","speed"},
    start=function(key)
        ...
        for i=1,layers do
            ...
            local mesh=Instance.new("SpecialMesh") mesh.MeshType=Enum.MeshType.Sphere mesh.Scale=Vector3.new(1,1.2,1) mesh.Parent=a
...
Visuals["portalring"] = {
    name="portal ring", desc="swirling portal ring around your waist", settings={"color","rainbow","size","opacity","speed"},
    start=function(key)
...
Visuals["sparkleburst"] = {
    name="sparkle burst", desc="random sparkles pop all over your body", settings={"color","rainbow","size","speed"},
    start=function(key)
        local timer=0
        Conns[key]=RunService.Heartbeat:Connect(function(dt)
            local hrp=getHRP() if not hrp then return end
...
for _, key in ipairs(VisualOrder) do
    Settings[key] = {color=Config.Accent,rainbow=false,neon=true,size=1.0,speed=1.0,opacity=0.7,count=6}
    Enabled[key] = false
end
...
local function enableVisual(key)
    ...
    Enabled[key]=true
    if Visuals[key] and Visuals[key].start then pcall(Visuals[key].start,key) end
end
local function disableVisual(key)
    if not Enabled[key] then return end
    Enabled[key]=false
    if Visuals[key] and Visuals[key].stop then pcall(Visuals[key].stop,key) end
end
...
-- ════════════════════════════════════════════════════════════════════════════
-- ── GUI  (wider 600px, shorter 480px)
-- ════════════════════════════════════════════════════════════════════════════

local W, H = 600, 480   -- ← change these two numbers to resize
...
local function openSettingsPanel(key)
    ...
    sg.AncestryChanged:Connect(function() if not sg.Parent then titleRainbow:Disconnect() end end)
    ...
    local function makeSlider(row, fillColor, initVal, maxVal, onChange)
        local lbl2=row:FindFirstChildOfClass("TextLabel")
        local bg=Instance.new("Frame")
        bg.Size=UDim2.new(0.45,0,0,6) bg.Position=UDim2.new(0.52,0,0.5,-3)
        bg.BackgroundColor3=Config.Border bg.BorderSizePixel=0 bg.Parent=row corner(bg,3)
        local fill=Instance.new("Frame")
        fill.Size=UDim2.new(initVal/maxVal,0,1,0) fill.BackgroundColor3=fillColor
        fill.BorderSizePixel=0 fill.Parent=bg corner(fill,3)
        local handle=Instance.new("TextButton")
        handle.Size=UDim2.new(0,14,0,14) handle.Position=UDim2.new(initVal/maxVal,-7,0.5,-7)
        handle.BackgroundColor3=Config.Text handle.Text="" handle.BorderSizePixel=0
        handle.Parent=bg corner(handle,7)
    ...
    if table.find(supportedSettings,"color") then
        ...
        local colorPresets={
            Color3.fromRGB(255,80,80),Color3.fromRGB(255,160,40),Color3.fromRGB(255,230,40),
            Color3.fromRGB(80,220,80),Color3.fromRGB(40,180,255),Color3.fromRGB(120,80,255),
            Color3.fromRGB(255,80,180),Color3.fromRGB(255,255,255),Color3.fromRGB(0,255,120),Color3.fromRGB(20,20,20),
        }
        for ci,col in ipairs(colorPresets) do
            ...
            cb.MouseButton1Click:Connect(function()
                ...
                if Enabled[key] then disableVisual(key) task.wait(0.05) enableVisual(key) end
            end)
        end
    ...
    if table.find(supportedSettings,"size") then
        settingRow("size: "..string.format("%.1f",s.size),function(row)
            makeSlider(row,Config.Accent,s.size,3,function(rel,lbl2)
    ...
    if table.find(supportedSettings,"speed") then
        settingRow("speed: "..string.format("%.1f",s.speed),function(row)
            makeSlider(row,Config.AccentB,s.speed,3,function(rel,lbl2)
    ...
    if table.find(supportedSettings,"opacity") then
        settingRow("opacity: "..string.format("%.1f",s.opacity),function(row)
            makeSlider(row,Color3.fromRGB(200,200,100),s.opacity,1,function(rel,lbl2)
                s.opacity=rel
    ...
    if table.find(supportedSettings,"count") then
        settingRow("count: "..math.floor(s.count),function(row)
            makeSlider(row,Color3.fromRGB(255,120,200),s.count,20,function(rel,lbl2)
                s.count=math.floor(rel*20+0.5)
...
local function createMinimizedButton()
    ...
    RunService.Heartbeat:Connect(function() if btn.Parent then btn.BackgroundColor3=rainbowColor() end end)
    coroutine.wrap(function()
        while btn.Parent do
            tween(btn,1,{Size=UDim2.new(0,58,0,58)},Enum.EasingStyle.Sine):Play() task.wait(1)
            if not btn.Parent then break end
            tween(btn,1,{Size=UDim2.new(0,54,0,54)},Enum.EasingStyle.Sine):Play() task.wait(1)
        end
    end)()
    local clicked=false
    local function onClick()
        if clicked then return end clicked=true
        tween(btn,0.1,{Size=UDim2.new(0,44,0,44)}):Play() task.wait(0.1)
...
function createMainGui(fromMinimized, minSg, minBtn)
    if playerGui:FindFirstChild("VisualsWin") then playerGui.VisualsWin:Destroy() end
    if playerGui:FindFirstChild("VisualsMinimized") then playerGui.VisualsMinimized:Destroy() end
    ...
    if fromMinimized and minBtn then
        ...
        mainFrame.Size=UDim2.new(0,vs.X,0,vs.Y)
        mainFrame.Position=UDim2.new(0,vp.X,0,vp.Y)
        if minSg then minSg:Destroy() end
        tween(mainFrame,0.45,{Size=UDim2.new(0,W,0,H),Position=UDim2.new(0.5,-W/2,0.5,-H/2)},Enum.EasingStyle.Back):Play()
        tween(mainFrame,0.3,{BackgroundTransparency=0}):Play()
    else
        mainFrame.Position=UDim2.new(0.5,-W/2,0.5,-H/2-30)
        tween(mainFrame,0.4,{Position=UDim2.new(0.5,-W/2,0.5,-H/2),BackgroundTransparency=0}):Play()
    end

    -- Title bar
    local titleBar=Instance.new("Frame")
    titleBar.Size=UDim2.new(1,0,0,52) titleBar.BackgroundColor3=Config.Accent
    ...
    local titleLbl=Instance.new("TextLabel")
    titleLbl.Size=UDim2.new(1,-110,1,0) titleLbl.Position=UDim2.new(0,18,0,0)
    ...
    subLbl.Size=UDim2.new(1,-110,0,16) subLbl.Position=UDim2.new(0,18,1,-18)
    ...
    closeBtn.Size=UDim2.new(0,38,0,38) closeBtn.Position=UDim2.new(1,-46,0,7)
    ...
    searchBg.Size=UDim2.new(1,-24,0,32) searchBg.Position=UDim2.new(0,12,0,60)
    ...
    enableAllBtn.Size=UDim2.new(0.47,0,0,26) enableAllBtn.Position=UDim2.new(0,12,0,98)
    ...
    enableAllBtn.MouseButton1Click:Connect(function() for _,key in ipairs(VisualOrder) do enableVisual(key) end end)

    local disableAllBtn=Instance.new("TextButton")
    disableAllBtn.Size=UDim2.new(0.47,0,0,26) disableAllBtn.Position=UDim2.new(0.53,-12,0,98)
    ...
    disableAllBtn.MouseButton1Click:Connect(function() for _,key in ipairs(VisualOrder) do disableVisual(key) end end)

    -- Scroll list  — 2 columns to use the extra width
    local scroll=Instance.new("ScrollingFrame")
    scroll.Size=UDim2.new(1,-12,1,-134) scroll.Position=UDim2.new(0,6,0,130)
    ...
    gridLayout.CellSize=UDim2.new(0.5,-6,0,68)
    ...
    local function buildCard(key, order)
        local visual=Visuals[key] if not visual then return end
        local card=Instance.new("Frame")
        card.Name=key card.LayoutOrder=order
        ...
        swatch.Size=UDim2.new(0,4,1,-14) swatch.Position=UDim2.new(0,4,0,7)
        ...
        nameLbl.Size=UDim2.new(1,-90,0,20) nameLbl.Position=UDim2.new(0,14,0,6)
        ...
        descLbl.Size=UDim2.new(1,-90,0,30) descLbl.Position=UDim2.new(0,14,0,26)
        descLbl.BackgroundTransparency=1 descLbl.Text=visual.desc
        descLbl.TextColor3=Config.SubText descLbl.Font=Enum.Font.Gotham
        descLbl.TextSize=10 descLbl.TextXAlignment=Enum.TextXAlignment.Left
        descLbl.TextWrapped=true descLbl.Parent=card

        local settingsBtn=Instance.new("TextButton")
        settingsBtn.Size=UDim2.new(0,30,0,28) settingsBtn.Position=UDim2.new(1,-72,0.5,-14)
        settingsBtn.BackgroundColor3=Color3.fromRGB(25,25,40) settingsBtn.TextColor3=Config.SubText
        settingsBtn.Text="⚙" settingsBtn.TextSize=16 settingsBtn.Font=Enum.Font.GothamBold
        ...
        toggleBtn.Size=UDim2.new(0,38,0,28) toggleBtn.Position=UDim2.new(1,-36,0.5,-14)
        ...
        toggleBtn.BorderSizePixel=0 toggleBtn.Parent=card corner(toggleBtn,8) stroke(toggleBtn,1,Config.Off)
        ...
        local function refreshToggle()
            if Enabled[key] then
                ...
                local s2=toggleBtn:FindFirstChildOfClass("UIStroke") if s2 then s2.Color=Config.On end
            else
                ...
                local s2=toggleBtn:FindFirstChildOfClass("UIStroke") if s2 then s2.Color=Config.Off end
            end
        end
        refreshToggle()
        ...
        card.MouseEnter:Connect(function() if not Enabled[key] then tween(card,0.15,{BackgroundColor3=Color3.fromRGB(28,28,42)}):Play() end end)
        card.MouseLeave:Connect(function() if not Enabled[key] then tween(card,0.15,{BackgroundColor3=Config.Card}):Play() end end)
    end

    for i,key in ipairs(VisualOrder) do buildCard(key,i) end

    searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        local q=searchBox.Text:lower()
        for _,key in ipairs(VisualOrder) do
            local card=cardRefs[key] if not card then continue end
            card.Visible=(q=="" or Visuals[key].name:lower():find(q,1,true) ~= nil)
        end
    end)
